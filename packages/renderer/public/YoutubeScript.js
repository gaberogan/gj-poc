...
source.enable = (conf, settings, saveStateStr) => {
  ...
  if (!didSaveState) {
    ...
    if (isLoggedIn && !batchResp[1].isOk)
      throw new ScriptException('Failed to request context enable isLoggedIn && !batchResp[1].isOk')
...
source.getHome = () => {
  ...
  if (tabs[0].videos.length > 0) return new RichGridPager(tabs[0], {}, USE_MOBILE_PAGES, true)
...
source.searchSuggestions = (query) => {
  ...
  if (!suggestionsResp.isOk) throw new ScriptException('Failed to get suggestions')
...
source.search = function (query, type, order, filters) {
  ...
  if (searchResults.videos) return new SearchItemSectionVideoPager(searchResults)
  return []
}
source.searchChannels = function (query) {
  ...
  if (searchResults.channels) return new SearchItemSectionChannelPager(searchResults)
  return []
}
...
source.getContentDetails = (url, useAuth) => {
  ...
  if (videoId && _settings['youtubeDislikes']) batch.GET(URL_YOUTUBE_DISLIKES + videoId, {}, false)
  const resps = batch.execute()

  throwIfCaptcha(resps[0])
  if (!resps[0].isOk) throw new ScriptException('Failed to request page [' + resps[0].code + ']')
  ...
  const ageRestricted =
    initialPlayerData.playabilityStatus?.reason?.indexOf('your age') > 0 ?? false
  if (ageRestricted) {
  ...
  if (videoDetails == null) return new UnavailableException('No video found')
...
source.getContentChapters = function (url, initialData) {
  ...
  if (initialData == null) {
    ...
    if (resps[0].isOk && throwIfCaptcha(resps[0])) initialData = getInitialData(resps[0].body)
    else throw ScriptException('Failed to get chapters (' + resps[0].code + ')')
...
function mergeSBChapters(videoChapters, sbChapters) {
  ...
  for (let videoChapter of videoChapters) {
    const sponsors = sbChapters.filter(
      (x) => x.timeStart >= videoChapter.timeStart && x.timeStart <= videoChapter.timeEnd
    )
...
function extractVideoChapters(initialData) {
  ...
  if (rawObjects?.decoratedPlayerBarRenderer)
    rawObjects =
      rawObjects.decoratedPlayerBarRenderer?.playerBar?.multiMarkersPlayerBarRenderer?.markersMap
  else rawObjects = rawObjects.playerBar?.multiMarkersPlayerBarRenderer?.markersMap
  ...
  const validChapters = chapters.value.chapters.filter(
    (x) =>
      ...
      (x.chapterRenderer.timeRangeStartMillis || x.chapterRenderer.timeRangeStartMillis === 0)
  )
...
function getVideoDetailsHtml(url, useLogin) {
  ...
  else throw new ScriptException('Failed to get video details [' + url + '] (' + result.code + ')')
}
...
source.getLiveEvents = function (url) {
  ...
  if (!matchKey || matchKey.length < 1) throw new ScriptException('Live chat key not found')
...
source.getPlaybackTracker = function (url, initialPlayerData) {
  ...
  if (!initialPlayerData) {
    ...
    if (!initialPlayerData) throw new ScriptException('No playerData for playback tracker')
  }
...
class YoutubePlaybackTracker extends PlaybackTracker {
  ...
  onInit(seconds) {
    //Initial playback
    const resp = http.GET(
      constructUrl(this.playbackUrlBase, this.getProgressParameters(this.playbackParams, seconds)),
      getAuthContextHeaders(false),
      true
    )
    if (!resp.isOk) throw new ScriptException('Failed to initial playback:', resp.body)
  ...
  onProgress(seconds, isPlaying) {
    ...
    if (!resp.isOk) throw new ScriptException('Failed to update watchtime:', resp.body)
    else if (resp.body && resp.body.indexOf('ERROR'))
...
source.getComments = (url) => {
  ...
  const result = contents.twoColumnWatchNextResults?.results?.results?.contents ?? null //Add any alternative containers
...
source.getChannel = (url) => {
  ...
  if (!initialData) throw new ScriptException('No channel data found for: ' + url)
...
source.searchPlaylists = function (query, type, order, filters) {
  ...
  if (searchResults.playlists) return new SearchItemSectionPlaylistPager(searchResults)
  ...
  return switchKey(result.contents, {
    singleColumnBrowseResultsRenderer(renderer) {
      ...
      let playlistShelf = sections.find(
        (x) => x.shelfRenderer && extractText_String(x.shelfRenderer.title) == 'Playlists'
      )
        ...
        playlistShelf != null
          ...
          : sections.find(
              (x) =>
                ...
                extractText_String(
                  x.itemSectionRenderer.contents[0].horizontalCardListRenderer.header
      ...
      if (playlistShelf != null)
        ...
        playlistsItems =
          playlistItemSection.itemSectionRenderer.contents[0].horizontalCardListRenderer.cards
      else {
...
function removeQuery(urlPart) {
  ...
  if (urlPart.indexOf('?') > 0) return urlPart.substring(0, urlPart.indexOf('?'))
  else if (urlPart.indexOf('&') > 0) return urlPart.substring(0, urlPart.indexOf('&'))
  return urlPart
}
...
class YTLiveEventPager extends LiveEventPager {
  ...
  nextPage() {
    ...
    if (!newResult.isOk) throw new ScriptException('Failed chat: ' + newResult.body)
  ...
  for (let action of actions) {
    try {
      if (action.addChatItemAction) {
        ...
        if (msgObj.emojis)
          for (let emojiKey in msgObj.emojis) emojiMap[emojiKey] = msgObj.emojis[emojiKey]
        ...
        if (
          bannerRenderer &&
          redirectRenderer &&
          bannerRenderer.bannerType == 'LIVE_CHAT_BANNER_TYPE_CROSS_CHANNEL_REDIRECT'
        ) {
          const url =
            redirectRenderer.inlineActionButton?.buttonRenderer?.command?.commandMetadata
          ...
          if (url && name && thumbnails && thumbnails.length && thumbnails.length > 0)
            events.push(
              new LiveEventRaid(URL_BASE + url, name, thumbnails[thumbnails.length - 1]?.url)
            )
        }
      } else {
    ...
    if (this.continuation) {
      ...
      if (newData && newData.length > 0) {
        ...
        if (
          ...
          if (IS_TESTING) console.log('No videos in root found, checking shelves', newItemSection)
          let vids = []
  ...
  nextPage() {
    ...
    if (this.continuation) {
      ...
      if (continueItems.length > 0) {
        ...
        if (newItemSection.videos) return new SearchItemSectionVideoPager(newItemSection)
      }
    }
    this.hasMore = false
    this.results = []
  ...
  nextPage() {
    ...
    if (this.continuation) {
      ...
      if (continueItems.length > 0) {
        ...
        const newItemSection = extractSectionListRenderer_Sections(fakeSectionList, this.context)
        if (newItemSection.channels) return new SearchItemSectionChannelPager(newItemSection)
      }
    }
    this.hasMore = false
    this.results = []
  ...
  nextPage() {
    ...
    if (this.continuation) {
      ...
      if (continueItems.length > 0) {
        ...
        const newItemSection = extractSectionListRenderer_Sections(fakeSectionList, this.context)
        if (newItemSection.channels) return new SearchItemSectionPlaylistPager(newItemSection)
      }
    }
    this.hasMore = false
    this.results = []
...
function requestGuide(pageId) {
  if (!pageId) throw new ScriptException('No page id found, invalid authentication?')
...
function requestSearch(query, useAuth = false, params = null) {
  ...
  const resp = http.POST(
    URL_SEARCH + '?key=' + clientContext.INNERTUBE_API_KEY + '&prettyPrint=false',
    JSON.stringify(body),
    {
      'User-Agent': USER_AGENT_WINDOWS,
      'Content-Type': 'application/json',
...
function extractGuideEntry_AuthorLink(guideEntryRenderer) {
  ...
  const url = guideEntryRenderer.navigationEndpoint?.browseEndpoint?.canonicalBaseUrl
    ? URL_BASE + guideEntryRenderer.navigationEndpoint?.browseEndpoint?.canonicalBaseUrl
    : null
...
function extractChannel_PlatformChannel(initialData, sourceUrl = null) {
  ...
  if (!headerRenderer) {
    log(
      'Missing header renderer in structure: (' +
        sourceUrl +
        ')\n' +
  ...
  const thumbnail =
    thumbnails && thumbnails.length > 0
      ? thumbnails.sort(
          (a, b) =>
            Math.abs(a.width - thumbnailTargetWidth) - Math.abs(b.width - thumbnailTargetWidth)
        )[0]
      : { url: '' }
  ...
  const banner =
    banners && banners.length > 0
      ? banners.sort(
          (a, b) => Math.abs(a.width - bannerTargetWidth) - Math.abs(b.width - bannerTargetWidth)
        )[0]
      : { url: '' }
  ...
  return new PlatformChannel({
    ...
    subscribers: Math.max(
      0,
      extractHumanNumber_Integer(extractText_String(headerRenderer.subscriberCountText))
    ),
    description: '',
    url: idUrl,
...
function extractVideoPage_VideoDetails(initialData, initialPlayerData, contextData, jsUrl) {
  const contents = initialData.contents
  const contentsContainer = contents.twoColumnWatchNextResults?.results?.results ?? null
  if (!contentsContainer || !contentsContainer.contents || !initialPlayerData.videoDetails)
    return null
  ...
  const video = {
    ...
    thumbnails: new Thumbnails(
      videoDetails.thumbnail?.thumbnails.map((x) => new Thumbnail(x.url, x.height)) ?? []
    ),
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, videoDetails.channelId, config.id, PLATFORM_CLAIMTYPE),
      videoDetails.author,
      ...
      ? new UnMuxVideoSourceDescriptor(
          initialPlayerData.streamingData.adaptiveFormats
            ...
            .map((y) => {
              ...
              if (logItag) {
                log(
                  videoDetails.title + ' || Format ' + container + ' - ' + y.itag + ' - ' + y.width
                )
                log(
                  'Source Parameters:\n' +
                    JSON.stringify(
            ...
            return {
              ...
              getSubtitles() {
                ...
                for (let i = 0; i < lines.length; i++) {
                  const line = lines[i]
                  const lineParsed =
                    /<text .*?start="(.*?)" .*?dur="(.*?)".*?>(.*?)<\/text>/gms.exec(line)
        ...
        if (renderer.viewCount?.videoViewCountRenderer?.viewCount?.simpleText)
          ...
          video.viewCount = parseInt(
            extractFirstNumber_Integer(
              extractRuns_String(renderer.viewCount?.videoViewCountRenderer?.viewCount?.runs)
            )
          )
        }
        if (
          ...
          renderer.videoActions.menuRenderer.topLevelButtons.forEach((button) => {
            switchKey(button, {
              ...
              segmentedLikeDislikeButtonViewModel(renderer) {
                if (IS_TESTING) console.log('Found new likes model:', renderer)
                ...
                if (buttonViewModel?.title) {
                  ...
                  if (!isNaN(num) && num >= 0) video.rating = new RatingLikes(num)
  ...
  const scheduledTime =
    initialPlayerData?.playabilityStatus?.liveStreamability?.liveStreamabilityRenderer?.offlineSlate
      ?.liveStreamOfflineSlateRenderer?.scheduledStartTime

  if (scheduledTime && !isNaN(scheduledTime)) video.datetime = parseInt(scheduledTime)
            ...
            commentThreadRenderer(renderer) {
              ...
              const replyContinuation =
                renderer.replies?.commentRepliesRenderer?.contents?.length == 1
                  ? renderer.replies.commentRepliesRenderer.contents[0]?.continuationItemRenderer
...
function extractVideoWithContextRenderer_Video(videoRenderer, contextData) {
  const liveBadges = videoRenderer.thumbnailOverlays?.filter(
    (x) => x.thumbnailOverlayTimeStatusRenderer?.accessibility?.accessibilityData?.label == 'LIVE'
  )
  ...
  if (!videoRenderer?.lengthText?.runs || !videoRenderer.publishedTimeText?.runs) isLive = true //If no length, live after all?
  ...
  if (isLive) {
    ...
    return new PlatformVideo({
      ...
      author: author,
      uploadDate: parseInt(
        extractAgoText_Timestamp(extractText_String(videoRenderer.publishedTimeText))
      ),
...
function extractVideoWithContextRenderer_AuthorLink(videoRenderer) {
  let id =
    videoRenderer.channelThumbnail?.channelThumbnailWithLinkRenderer?.navigationEndpoint
      ?.browseEndpoint?.browseId
  const name = extractRuns_String(videoRenderer.shortBylineText.runs)
  const channelThumbs =
    videoRenderer.channelThumbnail.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails
...
function extractVideoRenderer_AuthorLink(videoRenderer) {
  const id =
    videoRenderer.channelThumbnailSupportedRenderers.channelThumbnailWithLinkRenderer
...
function searchQueryToSP(sort, type, filters) {
  ...
  let filter_duration =
    filters?.duration && filters.duration.length > 0 ? filters.duration[0] : null
...
function decryptN(encryptedN, jsUrl) {
  if (!_nDecrypt[jsUrl]) throw new ScriptException('N Decryptor was not available [' + jsUrl + ']')
...
function testCiphers() {
  ...
  for (hash of CIPHER_TEST_HASHES) {
    const jsUrl = CIPHER_TEST_PREFIX + hash + CIPHER_TEST_SUFFIX
    try {
      if (prepareCipher(jsUrl)) testResults.push('CipherTest [' + hash + ']: PASSED')
...
function prepareCipher(jsUrl) {
  ...
  try {
    ...
    if (!playerCodeResp.isOk) throw new ScriptException('Failed to get player js')
...
function getNDecryptorFunctionCode(code, jsUrl) {
  ...
  if (!nDecryptFunctionArrNameMatch) throw new ScriptException('Failed to find n decryptor (name)')
  ...
  if (!nDecryptFunctionNameMatch) throw new ScriptException('Failed to find n decryptor (array)')
  ...
  if (!nDecryptFunctionCodeMatch) throw new ScriptException('Failed to find n decryptor (code)')
...
function getCipherFunctionCode(playerCode, jsUrl) {
  ...
  if (!cipherFunctionName) throw new ScriptException('Failed to find cipher (name)')
  const cipherFunctionCodeMatch = playerCode.match(
    '(' + cipherFunctionName.replace('$', '\\$') + '=function\\([a-zA-Z0-9_]+\\)\\{.+?\\})'
  )
  if (!cipherFunctionCodeMatch) {
    if (IS_TESTING) console.log('Failed to find cipher function in: ', playerCode)
  ...
  const functionCode =
    'return function decodeCipher(str){ return ' + cipherFunctionName + '(str); }'
...
function parseQueryString(query) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    else results[part.substring(0, valueIndex)] = part.substring(valueIndex + 1)
  }
  return results
}
//#endregion
